...
data_scores <- data_scores |>
  remove_problematic(remove = "Symmetry.ACC <= acc_criterion", 
                     log_file = here("data/logs/symspan_problematic.csv")) |>
  replace_outliers(variables = "SymSpan.EditDistanceScore", 
                   cutoff = outlier_criterion, with = "NA", pass = 2,
                   log_file = here("data/logs/symspan_outliers.csv")) |>
  filter(!is.na(SymSpan.EditDistanceScore))
# ------------------------------------------------------------------------------
...
splithalf <- reliability |>
  ...
  summarise(r_partial = 
            ...
            r_editdistance = 
              cor(SymSpan.EditDistanceScore_odd, 
                  SymSpan.EditDistanceScore_even)) |>
  mutate(r_partial = (2 * r_partial) / (1 + r_partial),
         r_editdistance = (2 * r_editdistance) / (1 + r_editdistance))

data_scores$SymSpan.PartialScore.splithalf <- splithalf$r_partial
data_scores$SymSpan.EditDistanceScore.splithalf <- splithalf$r_editdistance
...
data_scores$SymSpan.PartialScore.cronbachalpha <- 
...
data_scores$SymSpan.EditDistanceScore.cronbachalpha <- 
