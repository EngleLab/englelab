...
data_scores <- data_scores |>
  remove_problematic(remove = "Rotation.ACC <= acc_criterion", 
                     log_file = here("data/logs/rotspan_problematic.csv")) |>
  replace_outliers(variables = "RotSpan.EditDistanceScore", 
                   cutoff = outlier_criterion, with = "NA", pass = 2,
                   log_file = here("data/logs/rotspan_outliers.csv")) |>
  filter(!is.na(RotSpan.EditDistanceScore))
# ------------------------------------------------------------------------------
...
splithalf <- reliability |>
  ...
  ungroup() |>
  summarise(r_partial = 
              cor(RotSpan.PartialScore_odd, RotSpan.PartialScore_even),
            r_editdistance = 
              cor(RotSpan.EditDistanceScore_odd, 
                  RotSpan.EditDistanceScore_even)) |>
  mutate(r_partial = (2 * r_partial) / (1 + r_partial),
         r_editdistance = (2 * r_editdistance) / (1 + r_editdistance))

data_scores$RotSpan.PartialScore.splithalf <- splithalf$r_partial
data_scores$RotSpan.EditDistanceScore.splithalf <- splithalf$r_editdistance
...
data_scores$RotSpan.PartialScore.cronbachalpha <- 
...
data_scores$RotSpan.EditDistanceScore.cronbachalpha <- 
