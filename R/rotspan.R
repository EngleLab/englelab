...
raw_rotspan <- function(x, blocks = "", taskVersion = "new"){
  ...
  if (blocks==1){
    x <- dplyr::mutate(x,
                       Block =
                         dplyr::case_when(`Running[Trial]`=="BlockList1" ~ 1,
                                                TRUE ~ as.numeric(NA)),
    ...
    x <- dplyr::group_by(x, Subject, Block, Trial)
    supressMessages({
      x <- dplyr::mutate_at(x, dplyr::vars(-group_cols()),
                            .funs = zoo::na.locf, na.rm = FALSE)
    })
    x <- dplyr::mutate(x,
    ...
    supressWarnings({
      x <- dplyr::mutate(x,
                         ...
                         Processing.total =
                           stats::ave(Processing.total,
                                      FUN = function(x) sum(x, na.rm = TRUE)),
                         Recall.total =
                           ifelse(SubTrialProc=="Recall", Accuracy, NA),
                         Recall.total =
                           stats::ave(Recall.total,
                                      FUN = function(x) sum(x, na.rm = TRUE)))
    })
                       ...
                       RT =
                         dplyr::case_when(SubTrialProc=="ProcessingTask" &
                                            ...
                                            as.double(CheckResponse1.RT),
                                             TRUE ~ as.double(NA)),
                       AvgRotationTime = ifelse(!is.na(AvgRotationTime) &
                                                  AvgRotationTime=="?", NA,
                                                AvgRotationTime))
    ...
    suppressWarnings({
      x <- dplyr::mutate(x,
                         ...
                         position_7 = ifelse(SubTrial==7, ArrowId, NA),
                         position_7 = max(position_7, na.rm = TRUE),
                         memory_item = dplyr::case_when(serial.position==1 ~
                                                          as.integer(position_1),
                                                        serial.position==2 ~
                                                          as.integer(position_2),
                                                        serial.position==3 ~
    ...
    x <- dplyr::mutate(x,
                       ...
                       RT =
                         dplyr::case_when(SubTrialProc=="ProcessingTask" &
                                          ...
                                          TRUE ~ as.double(NA)),
                       AvgRotationTime =
                         ifelse(!is.na(AvgRotationTime)&AvgRotationTime=="?",
                                NA, AvgRotationTime))
