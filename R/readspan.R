...
raw_readspan <- function(x, include_col = c(), taskVersion = "new") {
  ...
  if (taskVersion == "new") {
    ...
    if (!("AvgReadTime" %in% colnames(x))) {
      x <- dplyr::mutate(x, AvgReadTime = NA)
    }
  }

  if (taskVersion == "old" | taskVersion == "oswald") {
    x <- dplyr::filter(x, `Procedure[Block]` == "SessionProc")
    x <- dplyr::mutate(x, ReadACC = NA, AvgReadTime = NA)
  }
        ...
        dplyr::mutate(x,
                      SENSEBOTH.RT =
                        dplyr::case_when(SubTrialProc == "ProcessingTask" &
                                           Block == 1 ~ as.double(SENSEBOTH.RT),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 2 ~ as.double(SENSEBOTH1.RT),
                                         TRUE ~ as.double(NA)),
                      showSentence.RT =
                        dplyr::case_when(SubTrialProc == "ProcessingTask" &
                                           Block == 1 ~ as.double(showSentence.RT),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 2 ~ as.double(showSentence1.RT),
                                         TRUE ~ as.double(NA)),
                      CollectClick.RT =
                      ...
                      SENSEBOTH.ACC =
                        dplyr::case_when(SubTrialProc == "ProcessingTask" &
                                           Block == 1 ~ as.double(SENSEBOTH.ACC),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 2 ~ as.double(SENSEBOTH1.ACC),
                                         TRUE ~ as.double((NA))))
    } else if (blocks == 3) {
      x <-
        dplyr::mutate(x,
                      SENSEBOTH.RT =
                        dplyr::case_when(SubTrialProc == "ProcessingTask" &
                                         ...
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 2 ~ as.double(SENSEBOTH1.RT),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 3 ~ as.double(SENSEBOTH2.RT),
                                         TRUE ~ as.double(NA)),
                      showSentence.RT =
                        dplyr::case_when(SubTrialProc == "ProcessingTask" &
                                           Block == 1 ~ as.double(showSentence.RT),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 2 ~ as.double(showSentence1.RT),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 3 ~ as.double(showSentence2.RT),
                                         TRUE ~ as.double(NA)),
                      CollectClick.RT =
                        dplyr::case_when(SubTrialProc == "Recall" &
                                           Block == 1 ~
                      ...
                      SENSEBOTH.ACC =
                        dplyr::case_when(SubTrialProc == "ProcessingTask" &
                                         ...
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 2 ~ as.double(SENSEBOTH1.ACC),
                                         SubTrialProc == "ProcessingTask" &
                                           Block == 3 ~ as.double(SENSEBOTH2.ACC),
                                         TRUE ~ as.double((NA))))
    }
  }
  ...
  x <- dplyr::mutate(x,
                     RT =
                       dplyr::case_when(SubTrialProc == "ProcessingTask" ~
                                          as.double(SENSEBOTH.RT) +
                                          as.double(showSentence.RT),
                                        SubTrialProc == "Recall" ~
                                          as.double(CollectClick.RT),
                                        TRUE ~ as.double(NA)),
                     clear_items =
                     ...
                     recall_response =
                       dplyr::case_when(SubTrialProc == "Recall" ~
                                        ...
                                        TRUE ~ as.character(NA)),
                     AvgReadTime =
                       ifelse(!is.na(AvgReadTime) &
                                AvgReadTime == "?",
                              as.numeric(NA), AvgReadTime),
                     remove =
                       dplyr::case_when(SubTrialProc == "Recall" &
...
#' Calculate Reading Span Scores
#'
#' Calculate various span scores from the output of `raw_readspan()`
#' @param x dataframe
#' @export
#'

score_readspan <- function(x) {
  ...
  x_recall <- dplyr::summarise(x_recall,
                               ReadSpan.EditDistanceScore = sum(EditDistance.load),
                               ReadSpan.EditDistanceUnit = mean(EditDistance.unit),
                               ReadSpan.EditDistanceLoad =
                                 sum(EditDistance.load) / sum(SetSize),
                               ReadSpan.PartialScore = sum(Partial.load),
                               ReadSpan.PartialUnit = mean(Partial.unit),
                               ReadSpan.PartialLoad =
                                 sum(Partial.load) / sum(SetSize),
                               ReadSpan.AbsoluteScore = sum(Absolute.load),
                               ReadSpan.AbsoluteUnit = mean(Absolute.unit),
                               ReadSpan.AbsoluteLoad =
                                 sum(Absolute.load) / sum(SetSize),
                               ReadSpan.Trials = dplyr::n(),
                               ReadSpan.MemoryItems = sum(SetSize))
  x_processing <- dplyr::filter(x, SubTrialProc == "ProcessingTask")
  x_processing <- dplyr::summarise(x_processing,
                                   Read.ACC = mean(Accuracy, na.rm = TRUE),
                                   Read.RT_mean = mean(RT, na.rm = TRUE),
                                   Read.RT_sd = sd(RT, na.rm = TRUE),
  ...
  x <- dplyr::relocate(x,
                       ReadSpan.Trials, ReadSpan.MemoryItems,
